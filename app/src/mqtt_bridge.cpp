...
mqtt_bridge_t::mqtt_bridge_t() noexcept
    ...
    , _mqtt_connected{}
    , _flunder_client{std::make_unique<flunder::client_t>()}
    , _mqtt_thread{}
    , _flunder_thread{}
{}
    ...
    -> void
{
    do {
        connect(
            "flunder",
            (int(flunder::client_t::*)(std::string_view, int))(&flunder::client_t::connect),
            _flunder_client.get(),
            flunder::FLUNDER_HOST,
            flunder::FLUNDER_PORT);
...
auto mqtt_bridge_t::flunder_receive_callback(
    flunder::client_t* flunder_client, const flunder::variable_t* var, const void* userp) //
    -> void
{
    if (var->topic().starts_with("/@")) {
    ...
    if (!mqtt_bridge->mqtt_connected()) {
        std::fprintf(
            stdout,
            "-- dropping flunder message %s as mqtt is not connected\n",
            var->topic().data());
        return;
    }
